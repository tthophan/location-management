...
import { formatMilliseconds, getTimestamp } from 'src/utils';
  ...
  implements NestInterceptor<T, SuccessResponse<T> | T>
{
  ...
  intercept(
    ...
    | Promise<Observable<SuccessResponse<T> | T>> {
      ...
      map((data) => {
        ...
        return new SuccessResponse<T>({
          data: instanceToPlain(data, {
            strategy: 'exposeAll',
            excludePrefixes: ['_', '__', 'createdBy', 'updatedBy', 'isDeleted'],
            exposeUnsetFields: true,
          }) as T,
          responseTime: formatMilliseconds(
            now - request.context?.requestTimestamp,
          ),
